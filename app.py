...
def write_and_run_temp_python(code: str, injected_pickle: str = None, timeout: int = 60) -> Dict[str, Any]:
    ...
    if injected_pickle:
        ...
        preamble.append("    print(f'DataFrame loaded successfully. Shape: {df.shape}', file=sys.stderr)\n")
        preamble.append("    print(f'DataFrame columns: {list(df.columns)}', file=sys.stderr)\n")
        preamble.append("    if len(df) > 0:\n")
        preamble.append("        print('DataFrame has data - analysis can proceed', file=sys.stderr)\n")
        preamble.append("    else:\n")
        preamble.append("        print('Warning: DataFrame is empty - no data for analysis', file=sys.stderr)\n")
        preamble.append("except Exception as e:\n")
        preamble.append("    print(f'Warning: Failed to load pickle: {e}', file=sys.stderr)\n")
        ...
        # Initialize df as empty DataFrame to prevent NameError
        preamble.append("df = pd.DataFrame()\n")
        # Add data validation info
        preamble.append("print(f'DataFrame shape: {df.shape}', file=sys.stderr)\n")
        preamble.append("print(f'DataFrame columns: {list(df.columns)}', file=sys.stderr)\n")
        preamble.append("print('DataFrame is empty - no data available for analysis', file=sys.stderr)\n")

    # Data validation helper
    data_helper = r'''
def check_data_availability():
    """Helper function to check if DataFrame has data and required columns"""
    if df is None or df.empty:
        return False, "DataFrame is None or empty"
    if len(df) == 0:
        return False, "DataFrame has no rows"
    return True, f"DataFrame has {len(df)} rows and {len(df.columns)} columns"

def safe_get_column(column_name, default=None):
...
'''
    ...
    script_lines.append(data_helper)
...
7. IMPORTANT: Always check if the DataFrame 'df' exists and has data before using it. If 'df' is empty or undefined, you must handle this gracefully.
8. Your code should work whether data is provided via upload or scraping.
9. CRITICAL: Before using 'df', always check:
   - if 'df' is not None
   - if 'df' is not empty (len(df) > 0)
   - if the required columns exist
   - Provide meaningful error messages if data is missing
10. Use the provided helper functions:
    - `check_data_availability()` to verify data exists
    - `safe_get_column(column_name)` to safely access columns
...
def run_agent_safely_unified(llm_input: str, pickle_path: str = None) -> Dict:
    ...
    try:
        ...
        print(f"DEBUG: Raw LLM output: {raw_out[:500]}...", file=sys.stderr)
